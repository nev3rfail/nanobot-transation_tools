...
def register(listen=True, config={}, ** kwargs):
    ...
    if listen:
            ...
            if len(segs) >= 2:
                ...
                if action == "default_to":
                    if param in google_languages:
                        ...
                        bot.send_message(message.chat.id, param + " is not supported. Supported languages are:\n" + ', '.join(google_languages))
        ...
        @bot.message_handler(func=lambda msg: check_if_i_needed(msg)) #any message in direct chat should trigger translation
...
def check_if_i_needed(msg):
    if msg.chat.type in ['group', 'supergroup', 'channel'] and ((msg.text or msg.caption or '').startswith('!translate ') or (msg.text or msg.caption or '').startswith('!tr ')):
        #trigger only with commands
        return True
    elif msg.chat.type == 'private' and not (msg.text or msg.caption or '').startswith('!help') and not (msg.text or msg.caption or '').startswith('/help'):
